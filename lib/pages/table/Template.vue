...
export default {
  ...
  methods: {
    ...
    // 创建规则
    createRules() {
      let tagId = this.$refs.groupTree.getCurrentNodeKey()
      this.dialog({
        title: '请设置基础数据',
        body: <RulesSet ref="RulesSet" group-id={tagId + ''} type="1" />,
        theme: 'dark',
    ...
    initByCache() {
      let params = window.sessionStorage.getItem('template-table-params')
      if (params) {
        try {
    ...
    saveCache() {
      ...
      storage.setItem('template-table-params', JSON.stringify(this.params))
    }
  }
}
</script>
